...
local function GetClosestPlayer()
	if not Environment.Locked then
		...
		for _, v in next, (game.Workspace.Bots:GetChildren()) do
			if v ~= LocalPlayer and v and v:FindFirstChild(Environment.Settings.LockPart) and v:FindFirstChildOfClass("Humanoid") then
				if Environment.Settings.AliveCheck and v:FindFirstChildOfClass("Humanoid").Health <= 0 then continue end
				...
				local Vector, OnScreen = Camera:WorldToViewportPoint(v[Environment.Settings.LockPart].Position); Vector = ConvertVector(Vector)
	...
	elseif (UserInputService:GetMouseLocation() - ConvertVector(Camera:WorldToViewportPoint(Environment.Locked[Environment.Settings.LockPart].Position))).Magnitude > RequiredDistance then
		CancelLock()
	end
end
